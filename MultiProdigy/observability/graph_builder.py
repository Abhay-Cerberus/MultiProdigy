...
class AgentGraphBuilder:
        ...
        for trace in traces:
            if trace.get('event_type') == 'message_sent':
                ...
                if agent_name:
                    ...
                    elif trace.get('status') == 'completed' and agent_stats[agent_name]['status'] != 'error':
                        agent_stats[agent_name]['status'] = 'active'
                    
                    agent_stats[agent_name]['last_seen'] = trace.get('start_time') or trace.get('timestamp')
    ...
    def _extract_edges(self, traces: List[Dict]) -> List[Dict]:
        ...
        for trace in traces:
            if trace.get('event_type') == 'message_sent':
                edge = {
                    'id': trace.get('message_id', f"{trace.get('sender')}-{trace.get('receiver')}-{len(edges)}"),
    ...
    def get_agent_details(self, agent_name: str) -> Dict[str, Any]:
        ...
        agent_traces = [
            t for t in traces 
            if t.get('agent_name') == agent_name or 
               t.get('sender') == agent_name or 
               t.get('receiver') == agent_name
        ]
        ...
        return {
            ...
            'message_history': [
                ...
                if t.get('event_type') == 'message_sent'
            ]
        }